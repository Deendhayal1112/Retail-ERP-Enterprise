...
export default class ChecklistPanel {
  constructor(options = {}) {
  ...
  render() {
    ...
    card.innerHTML = `
      <h3 class="rc-center-card-title">Go-Live Release Checklist</h3>
        ...
        ${this.checklist.map(item => `
          <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #F1F5F9; padding-bottom:12px;">
            <div>
              ...
              <span style="font-size:12px; color:#6B7280;">Checklist ID: ${item.id}</span>
            </div>
            <div style="display:flex; align-items:center; gap:12px;">
              <span class="rc-badge ${item.verified ? "passed" : "warning"}">
                ${item.verified ? "COMPLETED" : "PENDING"}
              </span>
              <button class="verify-chk-btn" data-id="${item.id}" style="height:32px; padding:0 12px; border:1px solid #E9EDF5; background:none; border-radius:6px; font-size:12px; font-weight:600; cursor:pointer; color:#1E293B;">
                Toggle Checklist
              </button>
            </div>
          </div>
    ...
    card.querySelectorAll(".verify-chk-btn").forEach(btn => {
